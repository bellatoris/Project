...
#        f=miniBatch_generate(list_dir, miniBatch_size=8)

# matlab import to python

# mlab.start() # should check does this line consumes much running time
"""
mlab = 4*[None]
for i in range(4):
    mlab[i] = Matlab(executable='/usr/bin/matlab')
    mlab[i].start()
"""
...
def dataLoad_SUN3D(mlab, sequenceName, nFrame1, nFrame2, dataTopPath, matlabCodePath='./SUN3Dflow_py.m', verbose=False):
   ...
   # print(sequenceName, nFrame1, nFrame2,dataTopPath, matlabCodePath,verbose)
    ...
    SUN3D_output = mlab.run_func(matlabCodePath, {
                                 'sequenceDir': dataTopPath, 'sequenceName': sequenceName, 'frameID': str(nFrame1), 'frameIDAnother': str(nFrame2)})
    #SUN3D_output = mlab.run_func(matlabCodePath,{'sequenceDir':dataTopPath,'sequenceName':'harvard_c11/hv_c11_2','frameID':'1','frameIDAnother':'2'})

    #a = mlab.run_func('./SUN3Dflow_py.m',{'sequenceDir':'','sequenceName':'/home/dongwoo/Project/dataset/SUN3D/hotel_sf/scan2/','frameID':str(1),'frameIDAnother':str(2)})
    output = SUN3D_output['result']
    img_height = output['img_height']
    img_width = output['img_width']
    ...
    output['image_first'] = np.array(
        output['image_first'].reshape(img_height, img_width, 3))
    output['image_second'] = np.array(
        output['image_second'].reshape(img_height, img_width, 3))
    output['opticalFlow'] = np.array(
        output['opticalFlow'].reshape(img_height, img_width, 2))
    output['egomotion'] = np.array(output['egomotion']).reshape(6)
    output['depth_first'] = np.array(
        output['depth_first'].reshape(img_height, img_width, 1))
    output['depth_second'] = np.array(
        output['depth_second'].reshape(img_height, img_width, 1))
    ...
    if verbose:
        ...
        scipy.misc.imsave('depth_first.png', output['depth_second'])
...
def readSUN3D_singleData(list_dir, threshold_PCE, threshold_camDist, threshold_overlap, threshold_iter, choose_secondFrame, output):
    ...
    while(True):
        ...
        if (frameID == frameIDAnother):
            ...
            while(True):
                frameIDAnother = random.randint(max(
                    minFrame, frameID-choose_secondFrame), min(maxFrame, frameID+choose_secondFrame))
                if (frameID != frameIDAnother):
                    break

            output_tmp = dataLoad_SUN3D(
                mlab, dataDir, frameID, frameIDAnother, '', './SUN3Dflow_py.m', False)
...
def miniBatch_generate(list_dir, miniBatch_size=8):
    ...
    choose_secondFrame = 50

    threshold_PCE = 0.1
    threshold_camDist = 0.03
    ...
    target_depth_first = torch.Tensor(
        miniBatch_size, height_resized, width_resized, 1).numpy()
    target_depth_second = torch.Tensor(
        miniBatch_size, height_resized, width_resized, 1).numpy()
    target_opticalFlow = torch.Tensor(
        miniBatch_size, height_resized, width_resized, 2).numpy()
    ...
    for iterProcess in range(int(miniBatch_size/numProcess)):
        ...
        print(procs)
        for iterBatch in range(numProcess):
            procs.append(Process(target=readSUN3D_singleData, args=(list_dir, threshold_PCE, threshold_camDist,
                         threshold_overlap, threshold_iter, choose_secondFrame, output_multiProcess[iterBatch])))
        ...
        for iterBatch in range(numProcess):
            output_tmp = output_multiProcess[iterBatch+processBias].get()
            output_multiProcess[iterBatch+processBias].close()

            input_image_first[iterBatch+processBias,
                              :, :, :] = output_tmp['image_first']
            input_image_second[iterBatch+processBias,
                               :, :, :] = output_tmp['image_second']
            target_depth_first[iterBatch+processBias,
                               :, :, :] = output_tmp['depth_first']
            target_depth_second[iterBatch+processBias,
                                :, :, :] = output_tmp['depth_second']
            target_opticalFlow[iterBatch+processBias,
                               :, :, :] = output_tmp['opticalFlow']
            target_egomotion[iterBatch+processBias,
                             :] = output_tmp['egomotion']

        for p in procs:
            p.join()

    return {'input_image_first': input_image_first, 'input_image_second': input_image_second, 'target_depth_first': target_depth_first, 'target_depth_second': target_depth_second, 'target_opticalFlow': target_opticalFlow, 'target_egomotion': target_egomotion}
...
#output = miniBatch_generate(outDir,8)
